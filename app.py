        ...
        for idx, keyword in enumerate(keywords):
            ...
            try:
                ...
                user_prompt = (
                    ...
                    "5. 마크다운(**, ##, - 등) 사용 금지. 대신 아래 서식 마커만 사용\n"
                    ...
                    "13. 서식 마커 사용법 (반드시 지켜주세요):\n"
                    "    - 소제목: <소제목>텍스트</소제목> (4단계 각 섹션 시작에 소제목 1개씩, 총 4~5개)\n"
                    "    - 강조: <강조>텍스트</강조> (핵심 메시지나 중요 포인트에 사용, 문단당 최대 1개)\n"
                    "    - 사진: <사진:설명> (사진이 들어갈 위치에 배치, 설명은 어떤 사진을 넣을지 구체적으로)\n"
                    "14. 사진 마커 배치 (총 5~8개): 도입 인사 직후 1장, 각 소제목 아래 1장씩, 세특/합격사례 전후, CTA 구간 2~3장\n\n"
                    "출력 형식:\n"
                    "[제목]\n(제목만 한 줄)\n\n"
                    "[본문]\n(서식 마커가 포함된 본문 전체)\n"
                )
    ...
    def worker():
        ...
        try:
            ...
            user_prompt = (
                ...
                "6. 마크다운(**, ##, - 등) 사용 금지. 대신 아래 서식 마커만 사용\n"
                ...
                "14. 서식 마커 사용법 (반드시 지켜주세요):\n"
                "    - 소제목: <소제목>텍스트</소제목> (4단계 각 섹션 시작에 소제목 1개씩, 총 4~5개)\n"
                "    - 강조: <강조>텍스트</강조> (핵심 메시지나 중요 포인트에 사용, 문단당 최대 1개)\n"
                "    - 사진: <사진:설명> (사진이 들어갈 위치에 배치, 설명은 어떤 사진을 넣을지 구체적으로)\n"
                "15. 사진 마커 배치 (총 5~8개): 도입 인사 직후 1장, 각 소제목 아래 1장씩, 업로드한 세특 사진 위치, CTA 구간 2~3장\n\n"
                "출력 형식:\n"
                "[본문]\n(서식 마커가 포함된 본문 전체)\n"
            )
